  ...
  const fetchReports = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/reports/reporter/${user.id}`
      );
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-slate-800 tracking-tight">My Reports</h2>
          <p className="text-slate-500 text-sm mt-0.5">{reports.length} total reports submitted</p>
        </div>
        <button
          onClick={fetchReports}
                ...
                <div className="p-5">
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {/* Assigned Hospital */}
                      {report.assignedHospital && (
                        <p className="text-teal-600 text-sm font-medium">
                          🏥 {report.assignedHospital.name} — 📞 {report.assignedHospital.phone}
                        </p>
                      )}
                    </div>
                  </div>
